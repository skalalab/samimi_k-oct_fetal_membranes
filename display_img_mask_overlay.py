...
def mirror_array(array, axis = 0):
    return np.flip(array,axis)

def _translations(image, labels, weights, show_image = False):
    
    # calculate amount to translate by using the mask
    
    #remove bottom layer
    mask_layers_only = labels
    mask_layers_only[labels == 5] = 0 # remove layer below placenta
    mask_flat = np.sum(mask_layers_only, axis=1) # flatten image into 1d array
    mask_valid_data_indices = np.argwhere(mask_flat > 0) # calculate indices where data > 0 (find borders)
    
    # calculate top translate value
    transpose_top_value = np.min(mask_valid_data_indices) # find bottom shift value
    
    # calculate bottom translate value
    transpose_bottom_idx = np.max(mask_valid_data_indices) # find top shift value
    mask_length = mask_flat.shape[0]
    transpose_bottom_value = mask_length - transpose_bottom_idx
    
    # translate mask and and image up
    translate_image_up = np.roll(image, -transpose_top_value, axis=0)
    translate_labels_mask_up = np.roll(labels, -transpose_top_value,axis=0)
    translate_weights_mask_up = np.roll(weights, -transpose_top_value, axis=0)
    
    if show_image:
        plt.imshow(translate_image_up)
        plt.imshow(translate_labels_mask_up)
        plt.imshow(translate_weights_mask_up)
    
    #translate bottom
    translate_image_down = np.roll(image, transpose_bottom_value,axis=0)
    translate_labels_mask_down = np.roll(labels, transpose_bottom_value,axis=0)
    translate_weights_mask_down = np.roll(weights, transpose_bottom_value,axis=0)
    if show_image:
        plt.imshow(translate_image_down)
        plt.imshow(translate_labels_mask_down)
        plt.imshow(translate_weights_mask_down)

    return

def rotate_image(image, degrees, show_image = False):
    rotated = skimage.transform.rotate(image, angle=degrees, preserve_range=True)
    if show_image:
        plt.imshow(rotated)
        plt.show()
    return rotated

def rotate_labels_mask(mask, degrees, show_image = False):
    rotated = skimage.transform.rotate(mask,angle=degrees, preserve_range=True)
