...
dir_name = path_matlab_csv.name.rsplit("_", 2)[0] # get dir name from Apex file
path_frame_vs_thickness = path_sample / dir_name
# path_frame_vs_thickness_csv = list(path_frame_vs_thickness.glob("*.csv"))[0]
path_frame_vs_thickness_csv = path_frame_vs_thickness / f"{dir_name}_thickness.csv"
df_frame_thickness = pd.read_csv(path_frame_vs_thickness_csv)


### check that they are the same length, extend if necessary
if not len(df_frame_apex_rise_pressure) == len(df_frame_thickness):
...
path_features_csv = path_sample / f"{dir_name}_features.csv"
